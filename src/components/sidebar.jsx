...
function ExpansionMenu({ title, icon, isOpen, children = [], onClick }) {
  return (
    <li className={`expansion-menu ${isOpen ? "open" : ""}`}>
      <div className="expansion-menu-title" onClick={onClick}>
        <img src={icon} alt="" className="menu-icon" height={18} width={18} />
        <span>{title}</span>
...
function NaviLink({ label, icon = null, onClick, isActive }) {
  return (
    <li className={`nav-link ${isActive ? "active" : ""}`} onClick={onClick}>
      {icon && <img src={icon} alt="" className="menu-icon" height={18} width={18} />}
      <span>{label}</span>
    </li>
  );
}
...
function Sidebar({ isOpen, onItemClick }) {
  ...
  React.useEffect(() => {
    ...
    else if (path === AppRoutes.leave) setOpenSubmenu("leave");
  }, [location.pathname]);

  return (
    <>
      {isOpen && <div className="sidebar-overlay" onClick={onItemClick} />}
        ...
        <div className="sidebar-menu">
          <ul>
            ...
            <ExpansionMenu
              ...
              isOpen={openSubmenu === "leave"}
              onClick={() => toggleSubmenu("leave")}
            >
              <ul>
                <NaviLink
                  label="Leave"
                  isActive={location.pathname === AppRoutes.leave}
                  onClick={() => {
                    handleNavigate(AppRoutes.leave);
                  }}
                />
              </ul>
            </ExpansionMenu>
